...
*	drop if treatment == 3
	...
	collapse (mean) shalive shwellalive shwellalive_red shwellalive_blue shalive_blue shalive_red statedSR statedSR_blue statedSR_red TreesAliveStated_Observed NumbTreesAliveObserved shBaobab shEpineux siteid ///
	...
	* Technical, just for PIF presentation:
	preserve
	encode parcelle, gen(parcel)
	tab bloc, gen(block)

	reg shwellalive shEpineux shBaobab GGFmember i.parcel i.siteid, robust
	restore
	XXX
	*Key analysis
	drop if treatment == 1
