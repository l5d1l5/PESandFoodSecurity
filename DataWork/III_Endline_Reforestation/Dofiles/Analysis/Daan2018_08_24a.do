* Verify directory path by running 
	...
	dis "`c(username)'" // The text that shows up is the username of your computer, insert that into the code below
...
save "DataWork\III_Endline_Reforestation\DataSets\Intermediate\DaanCollector_app_data_construct.dta", replace
...
sort region site bloc parcelle nom prenom // check for duplicates
quietly by region site bloc parcelle nom prenom:  gen dup = cond(_N==1,0,_n)
drop if objectid == 257 // objectid's 257 and 258 are identical, including starting time. Shows up in the actual payment file, so is treatment.

* Then merge the baseline survey to the actual paiement data made by CEMES
merge 1:1 region site bloc parcelle nom prenom using "DataWork\III_Endline_Reforestation\DataSets\Intermediate\actual paiement made_clean.dta", gen(merge2) force

rename enqueteur EnqueteurSurvey
rename treatment treatOld

merge m:1 region site bloc parcelle using "DataWork\III_Endline_Reforestation\DataSets\Intermediate\DaanCollector_app_data_construct.dta", gen(merge3)
