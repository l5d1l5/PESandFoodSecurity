...
sjlog using simulatedmultivariate1, replace
set obs 1000
set seed 987654321
generate id=_n
expand 5
...
sjlog using simulatedmultivariate2, replace
sort id (tper)
local theta=1
by id: generate ystar=.35*x1 + .66*x2 + .25*x3 + 1.5*Instrument + .7 + `theta'*alpha + u_i if _n==1
...
sjlog using simulatedmultivariate3, replace
...
program define mpheckman_d0
	...
	qui:{
	by id: generate double `T' = (_n == _N)
	sort id (tper)
	tempvar k1 zb1
	by id: generate double `k1' = (2*$ML_y1[1]) - 1
	by id: generate double `zb1' = `pi'[1]
	forvalues r = 2/$T_max {
	tempvar k`r' xb`r'
	by id: generate double `k`r'' = (2*$ML_y1[`r']) - 1
	by id: generate double `xb`r'' = `beta'[`r']
	}
...
sjlog using simulatedmultivariate4, replace
matrix p=(2,3,5,7,11)
global dr = 100
global T_max=5
global T_min=3
...
sjlog using simulatedmultivariate5, replace
...
sjlog using simulatedmultivariate6, replace
ml model d0 mpheckman_d0 (y: y = ylag x1 x2 x3) (Init_Period: y = x1 x2 x3 Instrument) /lnsigma /lntheta, /*
*/ title(Multivariate RE Probit, $dr Halton draws) missing
ml init b0, copy
ml max
sjlog close, replace

sjlog using simulatedmultivariate7, replace
