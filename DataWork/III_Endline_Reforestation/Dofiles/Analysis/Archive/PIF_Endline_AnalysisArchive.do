...
********************************************************************************
	...
	* drop not found participants
	...
	desc rev_total_12mois Lrev_total_12mois W01_rev_total_12mois ///
				rev_total_12mois b10 b11 sec_occup ///
				b21 b22 W01_b22 W03_b22 W05_b22
	
	

	* Socio-demographics
	
	iebaltab  	sexe prim_occ agriculteur W01_b10 W01_b11												/// 
 				sec_occup	W01_b14 W01_b15,																///
				grpvar(statut) 																			///
				savetex("$EDLREFOR_outFin/delete_me.tex") 	  											///
				tblnote("† winsorized at the 1st and 99th percentile.") 								///
				total rowvarlabels pftest replace														///
	
	filefilter "$EDLREFOR_outFin/delete_me.tex" "$EDLREFOR_outFin/Socio_dems.tex",       				///                                                          
				from("/BShline") to("/BScline{1-8}") replace  
	...
	* Participation au program PIF * (statut == 1)
	...
	summ		b21 W01_b22												///
				usage_item_1  montant_usage_1							///
				usage_item_2  montant_usage_2							///
				usage_item_3  montant_usage_3							///
				usage_item_4  montant_usage_4							///
				usage_item_5  montant_usage_5							///
				usage_item_6  montant_usage_6							///
				usage_item_7  montant_usage_7							///
				usage_item_8  montant_usage_8							///
				usage_item_9  montant_usage_9							
	...
	filefilter "$EDLREFOR_outFin/delete_me.tex" "$EDLREFOR_outFin/Participation_PIF.tex",       				///                                                          
				from("/BShline") to("/BScline{1-8}") replace  
...
********************************************************************************
	...
	* Balance Table of Food Security indicator	
		
	iebaltab 	FoodExp LFoodExp HFIAS_score HFIA_cat_1  HFIA_cat_4 HHS HHScat1 HHScat2 HDDS, 	///
				grpvar(statut) grplabels(1 PES @  0 non-PES)  order(1 0)	///
				/// vce(cluster a2_site)										///										
				total rowvarlabels pftest  										///
				tblnote("") 													///
				savetex("$EDLREFOR_outFin/PES_paper/delete_me.tex") 						///
				replace

	filefilter  "$EDLREFOR_outFin/PES_paper/delete_me.tex" "$EDLREFOR_outFin/PES_paper/Food_Consumption_Indices.tex",        ///                                                          
                    from("/BShline") to("/BScline{1-8}") replace  
	...
	* Balance Table of food consumption shares	
	...
	filefilter  "$EDLREFOR_outFin/delete_me.tex" "$EDLREFOR_outFin/Food_Consumption_shares.tex",        ///                                                          
                    from("/BShline") to("/BScline{1-8}") replace 
	...
	* HISTOGRAM FOOD INSECURITY			

	* Colored bars
	graph bar oct_2017 - sept_2018,  blabel(bar, format(%9.1f) size(vsmall)) 	bargap(10) ///
						ytitle(Percent, size(small)) ///
						ytitle(, margin(medium)) ///
						legend (order(	1 	"Octobre 2017"     					///
										2 	"Novembre 2017"   					///
										3 	"Decembre 2017"                     ///
										4 	"Janvier 2018"                      ///
										5 	"Fevrier 2018"                		///
						...
						///title("Prevalence de l'Insecurite Alimentaire par Mois", size(large)) ///
						...
						plotregion(fcolor(white) ifcolor(white)) scale(*1)		
	gr export 	"$EDLREFOR_outFin/FoodInsecure_bymonth.eps",  replace	
...
foreach var in  FoodExp LFoodExp	{
	...
	esttab  ///
				...
				addnotes("The baseline outcomes represent respectively the food expenditure and log-food expenditure at baseline."	///
						 ...
						 "education, distance to the reforestation site, and membership in a forest management group.")				///
...
foreach var in  FoodExp LFoodExp	{
    ...
    * constraint define 1 [y1]x11 = [y2]x22
    ...
    mvtobit (expfoodgroup1 = treatment ) ///
			(expfoodgroup2 = treatment ) ///
			(expfoodgroup3 = treatment ) ///
			(expfoodgroup4 = treatment ) ///
			(expfoodgroup5 = treatment ) ///
			(expfoodgroup6 = treatment ) ///
			(expfoodgroup7 = treatment ) ///
			(expfoodgroup8 = treatment ) ///
			(expfoodgroup9 = treatment ), ///
			dr(20)  adoonly
			an 
			*constraints(1)	
...
foreach var in  expfoodgroup1 expfoodgroup2 expfoodgroup3 expfoodgroup4 expfoodgroup5 expfoodgroup6 expfoodgroup7 expfoodgroup8 expfoodgroup9	{
	...
	ivregress 2sls L`var' i.bloc i.parcelle  (L2FoodExp = treatment)
	eststo  `var'_1
	estadd	local Controls		"No"
	estadd	local Bloc			"Yes"	
...
* ------------------------------------------------------------------------------
	...
	esttab  ///
				...
				addnotes("The control covariates include age, gender, occupation, landholdings, revenue, size of the household, education," 		///
						 "distance to the reforestation site, and membership in a forest management group.")					///
				label r2 ar2 constant  											///
				replace
