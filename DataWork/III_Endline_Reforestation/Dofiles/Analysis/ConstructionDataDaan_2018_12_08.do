* Verify directory path by running 
   ...
   * ---------------------
   ...
   /* if c(username)=="soest"  {
	...
	if c(username)=="soest"  {
	   cd "D:\Surfdrive\Onderzoek\BurkinaFaso\ThresholdPaper\ConstructionData"										
	}
...
merge 1:1 region site bloc parcelle using "OriginalDataSets/self reported survival_clean.dta", gen(mergeA)
br site region treatment parcelle if mergeA ==2 // No verified (=observed) data for Nosebou (Boucle du Mouhoun) and Tapoaboopo (Est)
...
save "IntermediateFiles/DaanSurvivalData2018_12_07", replace
...
use "OriginalDataSets/participant_survey_clean.dta", clear // Baseline survey
...
*save "DataWork/II_Baseline_Reforestation/DataSets/Intermediate/Daan_participant_survey_clean2018_12_07.dta", replace
save "IntermediateFiles/Daan_participant_survey_clean2018_12_07.dta", replace
...
*merge 1:1 region site bloc parcelle nom prenom using "DataWork/II_Baseline_Reforestation/DataSets/Intermediate/Daan_participant_survey_clean2018_12_07.dta", gen(mergeB)
merge 1:1 region site bloc parcelle nom prenom using "IntermediateFiles/Daan_participant_survey_clean2018_12_07.dta", gen(mergeB)
...
* READING IN THE FINAL DATA NOT DONE YET
*merge 1:1  hhid using "DataWork\III_Endline_Reforestation\DataSets\Intermediate\Clean\PIF_Endline_SectionB_clean.dta", keepusing(repondent_fullname b*)

/*use "OriginalDataSets/PIF_Endline_SectionB_clean.dta", clear
rename b1 nom
rename b2 prenom
rename a1_region region
rename a2_site site
rename a3_village village
rename a4_bloc bloc
rename a5_parcelle parcelle
save "IntermediateFiles/PIF_Endline_SectionB_cleanDaan.dta", replace
...
sort dup 
sum dup
br nom prenom region site village bloc parcelle dup if dup > 0
sort  nom prenom region site village bloc parcelle // there are still records that are very similar (same location, different parcelle), but seem different individuals
* CLARA, PLEASE TRY TO FIGURE OUT WHAT IS THE MATTER HERE -- SAME PEOPLE TWICE, OR DIFFERENT PEOPLE LIVING IN THE SAME VILLAGE?


merge 1:1 region site bloc village parcelle nom prenom using "IntermediateFiles/Daan_participant_survey_clean2018_12_07.dta", gen(mergeB)
*/
...
merge m:1 region site bloc parcelle using "IntermediateFiles/DaanSurvivalData2018_12_07.dta", gen(mergeC)

*save "DataWork/III_Endline_Reforestation/DataSets/Intermediate/DaanFullDataSet2018_12_07", replace
save "IntermediateFiles/DaanFullDataSet2018_12_07", replace
