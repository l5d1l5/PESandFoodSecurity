...
* test of cheating
		** Overreported survival rates
	preserve 
	drop if blocid == .
	drop if shalive == .
	drop if treatment == 3
	gen ShAliveOverrep = statedSR - shalive
	sum ShAliveOverrep if treatment == 1
	sum ShAliveOverrep if treatment == 2
	twoway histogram ShAliveOverrep if treatment == 1, width(0.1) start(-0.6) bfcolor(blue) blcolor(blue) barw(0.04)  freq ///
		|| histogram ShAliveOverrep if treatment == 2, width(0.1) start(-0.6) bfcolor(none) blcolor(red) barw(0.08) freq legend(order(1 "Linear payments" 2 "Threshold payments"))
	keep ShAliveOverrep statedSR shalive blocid treatment
	reshape wide ShAliveOverrep statedSR shalive, i(blocid) j(treatment)
	ttest ShAliveOverrep1 = ShAliveOverrep2
	ttest shalive1 = statedSR1
	ttest shalive2 = statedSR2
	restore
	
	** Overreported money
	generate NumbTreesAliveObserved = int(shalive * arbres_plante)
	gen PaymentAsked = 300 * arbres_vivant if treatment == 1
	...
	gen PaymentEarned = 300 * NumbTreesAliveObserved if treatment == 1
	...
	twoway histogram PaymentOverasked if treatment == 1, width(10000) start(-70000) bfcolor(blue) blcolor(blue) barw(3000)  freq ///
		|| histogram PaymentOverasked if treatment == 2, width(10000) start(-70000) bfcolor(none) blcolor(red) freq legend(order(1 "Linear payments" 2 "Threshold payments"))
	...
	keep PaymentOverasked PaymentEarned PaymentAsked blocid treatment
	reshape wide PaymentOverasked PaymentEarned PaymentAsked, i(blocid) j(treatment)
	ttest PaymentOverasked1 = PaymentOverasked2
	ttest PaymentAsked1 = PaymentEarned1
	ttest PaymentAsked2 = PaymentEarned2
	restore
...
use "DataWork\III_Endline_Reforestation\DataSets\Intermediate\DaanFullDataSet2018_12_07", clear
		...
		*drop if treatment == 3
		...
		ttest b27, by(treatment) // MORE discussion meetings in threshold treatment, but NOT significantly so (p = 0.3406)
		sum b28 if treatment == 1
		sum b28 if treatment == 2
		ttest b28, by(treatment) // MORE trips to the field to do maintenance in threshold treatment, but NOT significantly so (p = 0.697)
		...
		ttest meanMaintenanceGr, by(treatment) // Threshold just fails to implement significantly LOWER number of different maintenance techniques (p = 0.1060)
		...
		tab b34 treatment, chi2 // Evaluation of the amount of cooperation within the group
